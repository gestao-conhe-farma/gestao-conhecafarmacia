...
/**
 * Equipa completa (directoria): todos os membros autenticados podem ler
 * nome, email, papel e contactos — RLS policy "ler equipa".
 */
export async function listarEquipa() {
  const supabase = await createClient()
  const { data, error } = await supabase
    .from('pessoas')
    .select('id, nome, email, role, telefone, whatsapp, criado_em')
    .order('criado_em', { ascending: true })
...
/** Pessoa da equipa por id (perfil). null se não existir. */
