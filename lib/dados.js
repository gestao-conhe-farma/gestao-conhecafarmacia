...
/**
 * Última carta de cada tipo (prefixo CF-XXX) + próximo número por tipo
 * para o ano corrente. Alimenta o painel "últimas cartas por tipo" na
 * página de documentos: toda a equipa vê qual o número a usar na carta
 * seguinte. Usa SQL em bruto (agrupar por prefixo do código em Postgres
 * via supabase-js não é expressivo o suficiente).
 */
export async function listarUltimasCartas() {
  const supabase = await createClient()
  const { data, error } = await supabase.rpc('ultimas_cartas_por_tipo')
  if (error) {
    // Painel é um extra: página de documentos não deve rebentar se a
    // função SQL ainda não existir (migração por aplicar).
    console.error('[dados] últimas cartas por tipo:', error.message)
    return []
  }
  return data ?? []
}
...
export async function listarAnuncios() {
  ...
  const { data, error } = await supabase
    .from('anuncios')
    .select(
      'id, titulo, corpo, link, criado_em, expira_em, ativo, reuniao_id, reuniao:reunioes(id, titulo), autor:pessoas(id, nome)'
    )
    .eq('ativo', true)
    .order('criado_em', { ascending: false })
    .limit(50)
